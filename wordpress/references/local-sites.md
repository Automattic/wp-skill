...
(state lives in `./.playground/`); call the script by its absolute path under this skill's
`scripts/` directory.
...
- **The local URL comes from `.playground/server.port`**:
  `http://127.0.0.1:$(cat .playground/server.port)`. **Never read `siteurl` from the
...
- **Frontend checks need a cookie jar.** Playground answers a one-time 302 that sets a
  ...
  `ensure`):
  ```bash
  curl -sL -c .playground/cookies -b .playground/cookies "http://127.0.0.1:$(cat .playground/server.port)/"
  ```
...
- **`wp db export` silently no-ops on Playground** (exit 0, no file — SQLite has no
  ...
  `$(cat .playground/site-dir)/wp-content/database/.ht.sqlite`. Real `wp db export` exists
  only on real hosts.
